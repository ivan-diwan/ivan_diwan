...
class QSliderSmokeTests(unittest.TestCase):
    @classmethod
    ...
    def test_qslider_appears_in_palette_and_can_be_created(self) -> None:
        palette_types = {definition.type_name for definition in self.registry.list_palette_types()}
        self.assertIn("QSlider", palette_types)

        entity = self.controller.create_entity_from_drop("QSlider", 80, 90, "form_root")
        self._process_events()

        self.assertEqual(entity.type, "QSlider")
        ...
        self.assertIsInstance(view.inner_widget, QSlider)

    def test_qslider_properties_update_canvas_and_snapshot(self) -> None:
        entity = self.controller.create_entity_from_drop("QSlider", 80, 90, "form_root")
        self.controller.update_entity_property(entity.id, "orientation", "vertical")
        ...
        self.assertEqual(updated.properties["orientation"], "vertical")
        self.assertEqual(updated.properties["value"], 25)
        ...
        slider_widget = self.window._form_window._canvas._item_views_by_id[entity.id].inner_widget
        self.assertEqual(slider_widget.orientation(), Qt.Orientation.Vertical)
        self.assertEqual(slider_widget.minimum(), 0)
        self.assertEqual(slider_widget.maximum(), 100)
        self.assertEqual(slider_widget.singleStep(), 5)
        self.assertEqual(slider_widget.value(), 25)
        self.assertIn('"type": "QSlider"', self.controller.get_snapshot())

    def test_qslider_normalization_keeps_document_valid(self) -> None:
        entity = self.controller.create_entity_from_drop("QSlider", 80, 90, "form_root")

        self.controller.update_entity_property(entity.id, "orientation", "diagonal")
        self.controller.update_entity_property(entity.id, "minimum", 80)
        self.controller.update_entity_property(entity.id, "maximum", 20)
        self._process_events()

        updated = self.controller.active_document.get_entity(entity.id)
        self.assertEqual(updated.properties["orientation"], "horizontal")
    ...
    def test_qslider_json_and_python_round_trip(self) -> None:
        entity = self.controller.create_entity_from_drop("QSlider", 80, 90, "form_root")
        self.controller.rename_entity(entity.id, "volume_slider")
        self.controller.update_entity_property(entity.id, "orientation", "vertical")
        self.controller.update_entity_property(entity.id, "value", 35)
        ...
        with managed_test_paths("tests\\_tmp_qslider.json", "tests\\_tmp_qslider.py") as (json_path, py_path):
            ...
            self.assertIn("QSlider", python_source)
            self.assertIn(".setOrientation(Qt.Vertical)", python_source)
            ...
            self.assertIn(".setValue(35)", python_source)
